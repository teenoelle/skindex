...
type Sentiment = "strong_caution" | "caution" | "benefit" | "neutral";
type Note = { dimensions: string[]; climate: string[]; sentiment: Sentiment; text: string };

// Valid values must match Scanner.tsx SkinType / ClimateType unions
// Skin: oily | dry | reactive | damaged_barrier | acne_prone | mature | hyperpigmentation_prone
// Climate: humid | dry_climate | cold | hot | high_uv

function generateNotes(ing: {
  status: string;
  ...
  if (sc === "Exfoliant" && fc !== "aha exfoliant") {
    // Guard against double-noting for AHA Exfoliant flagged ingredients
    notes.push({
      dimensions: [],
      climate: ["high_uv"],
      sentiment: "strong_caution",
  ...
  if (sc === "Peptide" && cat !== "firming") {
    // Skip if firming category already covers it — both note mature+cold, avoid redundancy
    notes.push({
      dimensions: ["mature"],
      climate: ["cold", "dry_climate"],
      sentiment: "benefit",
  ...
  if (sc === "Plant Extract") {
    // Only note for reactive skin + hot since extracts are diverse
    notes.push({
      dimensions: ["reactive"],
      climate: ["hot"],
      sentiment: "caution",
