...
type RawProduct = { id: string; name: string; brand: string | null; image_url: string | null; ingredient_list: string | null; type?: string | null };

async function scoreProducts(products: RawProduct[], req: NextRequest, isRinseOff = false) {
...
export async function GET(req: NextRequest) {
  ...
  if (area) {
    const { data: areaTypes } = await supabase
      ...
      .select("name")
      .eq("body_area", area);
    const typeNames = (areaTypes ?? []).map((t) => t.name);
    if (!typeNames.length) return NextResponse.json({ products: [] });
    ...
    const results = await scoreProducts(products, req);
  ...
  const results = await scoreProducts(products, req, isRinseOff);
  return NextResponse.json({ products: results });
}
